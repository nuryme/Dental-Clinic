...
🔗 **Live site:** https://YOUR-VERCEL-URL-HERE  <!-- replace after deploying -->

A full-stack dental clinic web app: a patient-facing site with guest & account
booking, plus role-based dashboards for doctors and a super admin. Frontend and
...
- **Patient dashboard:** see your full **booking history** and **update your
  profile** (name, phone).

### As a doctor
- **Doctor dashboard** with your own bookings — filter by date, mark an
  appointment **done**.
- Stats: total patients, today's count, total revenue, and a last-7-days chart
  (Recharts).
...
### As the super admin
- **Manage Users:** see everyone and **promote a patient to a doctor**, giving
  them their own doctor dashboard.
...
- `PATCH /users/:id/role` *(super-admin)* — promote/demote between patient and doctor.
...
- `POST /appointments` — **public**; works for guests or logged-in patients.
  ...
  assigns a daily **serial** per doctor. Logged-in bookings are linked to the account.
...
- `GET /appointments/doctor/stats` *(doctor/super-admin)* — total patients,
  today's count, total revenue, and last-7-days counts/revenue. Super-admin sees
  clinic-wide totals; a doctor sees only their own.
